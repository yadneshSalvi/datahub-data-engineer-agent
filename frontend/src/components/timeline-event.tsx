...
export function TimelineEvent({ event, toolResult }: { event: RunEvent; toolResult?: ToolResultEvent }) {
  ...
  switch (event.kind) {
    ...
    case "tool_result":
      ...
      break;
    case "recall":
      content = <div className={cn("relative overflow-hidden rounded-xl border p-4", event.found === 0 ? "border-border bg-surface" : "border-info/55 bg-info/10 shadow-[0_0_34px_-20px_var(--color-info)]")}><span className={cn("absolute inset-y-0 left-0 w-1", event.found === 0 ? "bg-border-strong" : "bg-info")} /><div className="flex items-center gap-2"><span className={cn("grid size-8 place-items-center rounded-lg border", event.found === 0 ? "border-border bg-bg/35 text-fg-muted" : "border-info/35 bg-info/15 text-info")}><Lightbulb className="size-4" aria-hidden="true" /></span><div><p className={cn("text-xs font-bold", event.found === 0 ? "text-fg" : "text-info")}>{event.found === 0 ? "No prior post-mortems to recall" : <>Recalled {event.found} prior post-mortem{event.found === 1 ? "" : "s"}</>}</p><p className={cn("mt-0.5 text-[9px] font-bold uppercase tracking-[.14em]", event.found === 0 ? "text-fg-subtle" : "text-info/75")}>{event.found === 0 ? "Cold start · no prior findings" : "Institutional memory hit"}</p></div></div>{event.found > 0 && event.top ? <div className="mt-3 rounded-lg border border-info/20 bg-bg/35 p-3"><p className="font-mono text-[11px] font-semibold text-fg">{event.top.root_cause_name}</p><div className="mt-2 flex items-center gap-2"><Badge variant="info" className="py-0.5">{event.top.relevance.toFixed(1)}% relevant</Badge><span className="text-[10px] text-fg-muted">{event.top.hops_away} hops away</span><Link to={`/memory/${event.top.incident_id}`} className="ml-auto inline-flex items-center gap-1 text-[10px] font-semibold text-info hover:underline">Open memory<Link2 className="size-3" aria-hidden="true" /></Link></div></div> : <p className="mt-3 text-[11px] text-fg-muted">Cold start: no related post-mortem was found. The agent will build new memory.</p>}</div>;
      break;
    case "finding":
